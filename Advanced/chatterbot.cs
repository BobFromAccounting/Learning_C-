...
public class Chatbot
{
    ...
    static string randomFactsFile = "randomFacts.txt";
    static string knowledgeBaseFile = "knowledgeBase.txt";
    static string transposFile = "transpos.txt";
    static string[] randomFacts;
    static void Main()
    {
        string input = "";
        randomFacts = File.ReadAllLines(randomFactsFile);
        LoadKnowledgeFiles(out kb, knowledgeBaseFile);
        LoadKnowledgeFiles(out tp, transposFile);

        Console.Clear();

        Console.WriteLine("Hello, my name is Eliza!");
    ...
    static string CalculateResponse(string input)
    {
        ...
        {    string[] words = input.Split(' ');
            ...
            {
                if (tp.TryGetValue(word, out responses))
                {
                    Random rand = new Random();
